...
sub FragFileReader {
	...
	foreach my $line (@information){
		chomp($line);
		my @data=split(" ",$line);
		if($data[0]=~/X/gi ){ 	# if($data[0] eq "X"){
			push @array_integral, $data[4];
			push @array_basin,    $data[0];
	 		push @array_X,        $data[1];
	 		push @array_Y,        $data[2];
	 		push @array_Z,        $data[3];
	 		$aux_ba++;
...
sub cart_file_write {
	...
	for(my $j=0; $j<$num_basin;$j++){
		print FILE "$matrix[1][2][$j]\t$matrix[1][3][$j]\t$matrix[1][4][$j]\t$matrix[1][5][$j]\n";
...
sub basin_integral_multi {
	my ($m1, $m2,$basin1,$basin2, $car_pos, $car_neg)=@_;
	my @matrix1=@{$m1};
	my @matrix2=@{$m2};
	my @array_multi=();
	for(my $i=0; $i<$basin1; $i++){
		for(my $j=0; $j<$basin2; $j++){
			if (($matrix1[1][2][$i] eq $car_pos) and ($matrix2[1][2][$j] eq $car_neg)) {
				##print ("$matrix1[1][2][$i]  * $matrix2[1][2][$j] \n");
  ...
  #
	for (my $i = 0; $i < scalar (@{$big_array1[2]}); $i++) {
		for (my $j = 0; $j < scalar (@{$big_array2[2]}); $j++) {
			if (($big_array1[4][$i] eq $car_pos) and ($big_array2[4][$j] eq $car_neg)) {
				#print ("($big_array1[4][$i] eq $car_pos) and ($big_array2[4][$j] eq $car_neg)\n");
				...
				#
				my $key = $count;
				$HashCoordsAttra{$key} ="$big_array1[1][$i],$big_array1[2][$i],$big_array1[3][$i],$big_array1[4][$i] = $big_array2[1][$j],$big_array2[2][$j],$big_array2[3][$j],$big_array2[4][$j]";
...
sub position_change_fragment {
	...
	while (my $Fline = shift (@FragLines)) {
		...
		if($new_xyz[3]=~/X/gi ){ 	# if($new_xyz[3] eq "X"){
			push (@ar_x,$new_x);
			push (@ar_y,$new_y);
			push (@ar_z,$new_z);
			push (@ar_lab,$new_xyz[3]);
		}else{
			push (@coords,"$Atom_label\t$new_x\t$new_y\t$new_z");
		}
	}
  #
	my @bigarray = ([@coords],[@ar_x],[@ar_y],[@ar_z],[@ar_lab]);
	return @bigarray;
}
###################################
# Equation
...
sub write_xyz_file_header {
	...
	my ($pid, $atoms_total, $sum, $sum_pospos,$sum_posneg,$sum_negpos,$sum_negneg)=@_;
	open(XYZ,">>tmp$pid.tmp");
	print XYZ "$atoms_total\n";
	print XYZ "E = $sum\t$sum_pospos\t$sum_posneg\t$sum_negpos\t$sum_negneg\n";
	close (XYZ);
}
###################################
# write xyz file
...
sub sort_energies {
	...
	foreach my $file (@files){
		...
		$energy=~s/(E = )?//g;
		chomp($energy);
		my @tmp_arr = split '\t', $energy;
		push(@energies, $tmp_arr[0]);
		#
		push @sum_pospos_,$tmp_arr[1];
		push @sum_posneg_,$tmp_arr[2];
		push @sum_negpos_,$tmp_arr[3];
		push @sum_negneg_,$tmp_arr[4];
		#
		my $coords="";
		#
		foreach $a  (2..($#lines)){
	...
	#
	$avg_pospos = mean(@sum_pospos_);
	$avg_posneg = mean(@sum_posneg_);
	$avg_negpos = mean(@sum_negpos_);
	$avg_negneg = mean(@sum_negneg_);
	# Potencial
...
sub logo {
	print "\n";
	print "             __ __ _      __         __  _____________   \n";
	print "            / //_/(_)____/ /__      /  |/  / ____/ __ \\ \n";
	print "           / ,<  / / ___/ //_/_____/ /|_/ / __/ / /_/ /  \n";
	print "          / /| |/ / /__/ ,< /_____/ /  / / /___/ ____/   \n";
	print "         /_/ |_/_/\\___/_/|_|     /_/  /_/_____/_/       \n";
	print "\n\n";
	print "    Search global minimum structures of molecular clusters on the gas phase,\n";
	print "using as a Coulomb interaction between Molecular electrostatic potential (MEP)-based method\n";
	print "\n";
}
###################################
...
sub ConfigFileReader{
	...
	if (not defined $configfile) {
		die "\n Kick-MEP_Algorithm must be run with:\n\n Usage :\n\tperl $0 [configure-file]\n
		\n Help :\n\tperl $0 -help \n\n\n";
		exit(1);
	}
	#
...
sub resume_log_file {
	my ($na,$me,$timeS,$timeE,$timeT,$Raddi,$avg_pospos, $avg_posneg, $avg_negpos, $avg_negneg)=@_;
	#
	print "\n";
	print "  <> Summary Kick-MEP Algorithm <> \n\n";
	# Sample Size
	print "    * Sample Size: $Num_of_geometries\n";
	# Molecules f+ and f-
	print "    * Molecules \n";
	print "        Molfile 1 : $na.frag \n";
	print "        Molfile 2 : $me.frag \n";
	# Box size
	my $c_x = sprintf '%.2f', $Box_x;
	my $c_y = sprintf '%.2f', $Box_y;
	my $c_z = sprintf '%.2f', $Box_z;
	print "    * Box size : $c_x A x $c_y A x $c_z A\n";
	# Output file xyz
	print "    * Output File : $Final_File\n";
	print "    * Radius Sphere : $Raddi A\n";
	print "    Equation terms\n";
	$avg_pospos = sprintf '%.6f', $avg_pospos;
	$avg_posneg = sprintf '%.6f', $avg_posneg;
	$avg_negpos = sprintf '%.6f', $avg_negpos;
	$avg_negneg = sprintf '%.6f', $avg_negneg;
	print "    * Avg A+A+ : $avg_pospos \n";
	print "    * Avg A+A- : $avg_posneg \n";
	print "    * Avg A-A+ : $avg_negpos \n";
	print "    * Avg A-A- : $avg_negneg \n";

	my $J_value = sprintf '%.6f', $ValueJ[0];
	print "    * Minimum Coulombic-Interaction Value (J) : $J_value\n";

	#
	print "\n  <> Performance Data <> \n\n";
	# Total number of process
	print "    * Number of Process : $ncpus\n";
	...
	#
	print "MESSAGE Normal Termination Kick-MEP Algorithm\n\n\n";
	#
}
###################################
# Help
sub show_help {
	print " <> To use Kick-MEP Algorithm <> \n\n";
	#
	print "\n";
	...
	print " * File with the positive potential \n";
	print "\t Mol_1 = f+FileCoords.frag \n";
	print " * File with the negative potential \n";
	...
	print "\n";
	print " * Output xyz file name (Default name Kick-MEP-Output.xyz) \n";
...
###################################
# CHECK MinBOX: Box space search
sub box_space {
	# array are send by reference
	my ($coord_tmp,$coordmin,$coordmax) = @_;
	my $option;
	if ( ( $coord_tmp < $coordmax ) && ( $coord_tmp > $coordmin ) ) {
		$option = 0;
	} else {
		$option = 1;
	}
...
my @multi_pospos = basin_integral_multi(\@matrix_1, \@matrix_2, $basin_num_1, $basin_num_2, 'X+', 'X+');
my @multi_posneg = basin_integral_multi(\@matrix_1, \@matrix_2, $basin_num_1, $basin_num_2, 'X+', 'X-');
my @multi_negpos = basin_integral_multi(\@matrix_1, \@matrix_2, $basin_num_1, $basin_num_2, 'X-', 'X+');
my @multi_negneg = basin_integral_multi(\@matrix_1, \@matrix_2, $basin_num_1, $basin_num_2, 'X-', 'X-');
#
my $Raddi_Value = (1.0) * $weight_spherical;
#
foreach my $iteration(1 .. $Num_of_geometries){
	...
	###################################
	# Radial Constraint Algorithm (RCA for MEP)
	#
	#
	...
	my @dist_pospos = distance_medition(\@resume1,\@resume2, 'X+', 'X+');
	# Hash for multiplication
	for (my $i=0; $i<scalar (@multi_pospos); $i=$i+1){ my $key = $i; $HashMult{$key}=$multi_pospos[$i];}
	#print (scalar(@dist_pospos));
	#print ("\n");
	my @Mult_1    = ();
	my @Dist_1    = ();
	my @Attracs_1 = ();
	...
	foreach my $key (keys %HashMult){
		...
		my ($axis_x_1,$axis_y_1,$axis_z_1,$nameAtraA) = split ',', $Arr1;
		my ($axis_x_2,$axis_y_2,$axis_z_2,$nameAtraB) = split ',', $Arr2;
		my $dist_Frag1_Frag2 = (($axis_x_2-$axis_x_1)**2)+(($axis_y_2-$axis_y_1)**2)+(($axis_z_2-$axis_z_1 )**2);
		#
		if ( $dist_Frag1_Frag2 < $diameter) {
			#print (" $dist_Frag1_Frag2   $diameter \n");
			push (@Mult_1, $HashMult{$key});
			push (@Dist_1, $HashDist{$key});
			# Attractors
			push (@Attracs_1,"$nameAtraA\t$axis_x_1\t$axis_y_1\t$axis_z_1");
			push (@Attracs_2,"$nameAtraB\t$axis_x_2\t$axis_y_2\t$axis_z_2");
		}
	}
	my $sum_pospos = equation(\@Mult_1,\@Dist_1);
	# Delete data hash
	%HashCoordsAttra = ();
	%HashDist        = ();
	%HashMult        = ();
	...
	#
	# The measurement of the distance of the attractors is carried out
	my @dist_posneg = distance_medition(\@resume1,\@resume2, 'X+', 'X-');
	# Hash for multiplication
	for (my $i=0; $i<scalar (@multi_posneg); $i=$i+1){ my $key = $i; $HashMult{$key}=$multi_posneg[$i];}
	#print (scalar(@dist_posneg));
	#print ("\n");
	@Mult_1    = ();
	@Dist_1    = ();
	@Attracs_1 = ();
	...
	my $sum_posneg = equation(\@Mult_1,\@Dist_1);
	# Delete data hash
	%HashCoordsAttra = ();
	%HashDist        = ();
	%HashMult        = ();
	...
	#
	# The measurement of the distance of the attractors is carried out
	my @dist_negpos = distance_medition(\@resume1,\@resume2, 'X-', 'X+');
	# Hash for multiplication
	for (my $i=0; $i<scalar (@multi_negpos); $i=$i+1){ my $key = $i; $HashMult{$key}=$multi_negpos[$i];}
	#print (scalar(@dist_negpos));
	#print ("\n");
	@Mult_1    = ();
	@Dist_1    = ();
	@Attracs_1 = ();
	...
	my $sum_negpos = equation(\@Mult_1,\@Dist_1);
	# Delete data hash
	%HashCoordsAttra = ();
	%HashDist        = ();
	%HashMult        = ();
	...
	#
	# The measurement of the distance of the attractors is carried out
	my @dist_negneg = distance_medition(\@resume1,\@resume2, 'X-', 'X-');
	# Hash for multiplication
	for (my $i=0; $i<scalar (@multi_negneg); $i=$i+1){ my $key = $i; $HashMult{$key}=$multi_negneg[$i];}
	#print (scalar(@dist_negneg));
	#print ("\n");
	@Mult_1    = ();
	@Dist_1    = ();
	@Attracs_1 = ();
	...
	my $sum_negneg = equation(\@Mult_1,\@Dist_1);
	# Delete data hash
	%HashCoordsAttra = ();
	%HashDist        = ();
	%HashMult        = ();
	...
	#
	#
	my $total_sum = $sum_pospos + $sum_posneg + $sum_negpos + $sum_negneg;
	#print (" $total_sum = $sum_pospos + $sum_posneg + $sum_negpos + $sum_negneg;\n");
	# Write tmp file with Maximum Matching value and coordinates
	write_xyz_file_header($iteration,($atoms_num_1+$atoms_num_2),$total_sum, $sum_pospos,$sum_posneg,$sum_negpos,$sum_negneg);
...
#
# Outputfile is done
resume_log_file($without_extension_1,$without_extension_2,$datestringStart,$datestringEnd,timestr($tiempo_total),$Raddi_Value,$avg_pospos, $avg_posneg, $avg_negpos, $avg_negneg);
